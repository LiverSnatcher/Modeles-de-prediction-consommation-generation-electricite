...
# ==========================================
# Palette :
#   Fond app       #4e7c3c  vert printemps moyen
#   Fond sidebar   #3d6230  vert plus sombre
#   Cartes         #5c9048  vert clair surélevé
#   Texte principal #f0f8e8  crème très clair — contraste élevé
#   Texte secondaire #c0dfa8  vert clair lisible
#   Accent vert    #a8e060  vert vif printemps
#   Accent jaune   #d8c830  jaune printemps
#   Bordures       #6a9858  vert moyen
...
.main-sub {
    ...
    border-bottom: 1px solid #6a9858;
...
[data-testid="stSidebar"] {
    background-color: #3d6230 !important;
    border-right: 1px solid #6a9858 !important;
}
...
.stButton button[kind="primary"] {
    background: #d8c830 !important;
    border: none !important;
    color: #1e3010 !important;
}
.stButton button[kind="primary"]:hover {
    background: #e8d840 !important;
    color: #1e3010 !important;
}
.stButton button[kind="secondary"] {
    background: transparent !important;
    border: 1px solid #c0dfa8 !important;
    color: #f0f8e8 !important;
}
.stButton button[kind="secondary"]:hover {
    border-color: #f0f8e8 !important;
}

[data-testid="stMetric"] {
    background: #5c9048 !important;
    border: 1px solid #6a9858 !important;
    border-left: 4px solid #d8c830 !important;
...
[data-testid="column"]:nth-child(1) [data-testid="stMetric"] { border-left-color: #f0f8e8 !important; }
[data-testid="column"]:nth-child(2) [data-testid="stMetric"] { border-left-color: #a8e060 !important; }
[data-testid="column"]:nth-child(3) [data-testid="stMetric"] { border-left-color: #d8c830 !important; }
[data-testid="column"]:nth-child(4) [data-testid="stMetric"] { border-left-color: #c0dfa8 !important; }

[data-testid="stTabs"] [data-baseweb="tab-list"] {
    background: transparent !important;
    border-bottom: 2px solid #6a9858 !important;
    gap: 0 !important;
}
...
[data-testid="stTabs"] [data-baseweb="tab"]:hover { color: #f0f8e8 !important; }
[data-testid="stTabs"] [aria-selected="true"] {
    color: #d8c830 !important;
    border-bottom-color: #d8c830 !important;
}
...
hr { border-color: #6a9858 !important; margin: 1.2rem 0 !important; }
...
::-webkit-scrollbar-track { background: #3d6230; }
::-webkit-scrollbar-thumb { background: #6a9858; border-radius: 4px; }
::-webkit-scrollbar-thumb:hover { background: #a8e060; }
</style>
""", unsafe_allow_html=True)


# ==========================================
# 2. GESTION HISTORIQUE (SUPABASE ROLLING FORECAST)
# ==========================================
@st.cache_resource
...
def load_rolling_forecast():
    try:
        ...
        for key in ["conso", "eolien", "solaire", "residuelle"]:
            # Reconversion du JSONB en pd.Series avec index datetime
...
def stitch_forecasts(old_series, new_series, split_dt):
    """Garde le passé figé et y colle le nouveau futur."""
...
# ==========================================
# 3. FONCTIONS DE DONNÉES ET CACHE
# ==========================================
@st.cache_resource
def load_models():
...
def get_eco2mix_realtime():
    ...
    while total is None or offset < total:
        ...
        if not r.ok: raise RuntimeError("Erreur ODRE")
        data = r.json()
        if total is None: total = min(data["total_count"], 13 * 24 * 4)
...
def get_weather_all(start_dt, horizon_days):
    ...
    # Solaire V2 (4 Zones - Multi-features)
...
def bridge_pred(real: pd.Series, pred: pd.Series) -> pd.Series:
    """Ajoute le dernier point réel au début de la prévision pour continuité visuelle."""
...
PLOTLY_BASE = dict(
    ...
    plot_bgcolor="rgba(62,100,48,0.5)",
    paper_bgcolor="rgba(0,0,0,0)",
    font=dict(family="Nunito, sans-serif", size=11, color="#c0dfa8"),
    xaxis=dict(
        gridcolor="#4e7c3c",
        linecolor="#6a9858",
        tickfont=dict(size=10, color="#c0dfa8"),
        showspikes=True, spikecolor="#d8c830",
    ...
    legend=dict(
        bgcolor="rgba(45,75,35,0.95)",
        bordercolor="#6a9858",
        borderwidth=1,
        font=dict(size=10, color="#f0f8e8"),
    ),
    hoverlabel=dict(
        bgcolor="#3d6230",
        bordercolor="#6a9858",
        font=dict(family="Nunito, sans-serif", size=11, color="#f0f8e8"),
    ),
)
...
.main-sub {
    ...
    border-bottom: 1px solid #4a6e52;
    padding-bottom: 1rem;
}
...
.stButton button[kind="primary"] {
    background: #4db86a !important;
    border: none !important;
    color: #152018 !important;
}
.stButton button[kind="primary"]:hover {
    background: #70d488 !important;
    color: #152018 !important;
}
.stButton button[kind="secondary"] {
    background: transparent !important;
    border: 1px solid #5a8060 !important;
    color: #a8c8a0 !important;
}
.stButton button[kind="secondary"]:hover {
...
[data-testid="stMetric"] {
    background: #3a5c42 !important;
    border: 1px solid #4a6e52 !important;
    border-left: 4px solid #6ed98a !important;
    border-radius: 8px !important;
    padding: 1rem 1.2rem !important;
}
[data-testid="stMetricLabel"] {
...
/* Accent couleur par colonne métrique */
[data-testid="column"]:nth-child(1) [data-testid="stMetric"] { border-left-color: #d4785a !important; } /* Conso — terracotta */
[data-testid="column"]:nth-child(2) [data-testid="stMetric"] { border-left-color: #6ed98a !important; } /* Éolien — vert */
[data-testid="column"]:nth-child(3) [data-testid="stMetric"] { border-left-color: #e8c84a !important; } /* Solaire — or */
[data-testid="column"]:nth-child(4) [data-testid="stMetric"] { border-left-color: #90b8d8 !important; } /* Résiduelle — bleu doux */

[data-testid="stTabs"] [data-baseweb="tab-list"] {
    background: transparent !important;
    border-bottom: 2px solid #4a6e52 !important;
    gap: 0 !important;
}
[data-testid="stTabs"] [data-baseweb="tab"] {
    font-family: 'Nunito', sans-serif !important;
...
[data-testid="stTabs"] [aria-selected="true"] {
    color: #e0edd8 !important;
    border-bottom-color: #6ed98a !important;
}
[data-testid="stTabs"] [data-baseweb="tab-panel"] {
    background: transparent !important;
    padding-top: 1rem !important;
...
with st.sidebar:
    ...
    st.markdown("""
    <div style="font-family:'Nunito',sans-serif;font-size:0.75rem;color:#c0dfa8;line-height:2;">
    MODÈLE ÉOLIEN<br>
    <span style="color:#d8c830;font-weight:700;">TFT · 1 024 667 params</span><br>
...
if run_btn:
    with st.spinner("Acquisition et inférence en cours..."):
        ...
        if old_run and (last_known_dt - old_run['run_dt']).total_seconds() <= 3600:
            ...
            df_lags       = df_eco2mix_30[["consommation"]].rename(columns={"consommation": "value"}).dropna()
            df_renew_lags = (df_eco2mix_30[["eolien", "solaire"]]
        ...
        for i in range(1, steps_totaux + 1):
            ...
            past_c  = df_working_c[df_working_c.index <= target_dt - timedelta(minutes=30)]["value"].dropna()
            feat_c  = pd.DataFrame([{
                "heure": target_dt.hour, "jour_semaine": target_dt.weekday(), "mois": target_dt.month,
                "est_ferie": int(target_dt.date() in fr_holidays), "temperature": temperature,
            ...
            past_s_48 = df_working_s[df_working_s.index <= target_dt - timedelta(hours=24)]["value"].dropna()
            def sol_lag(h): return safe_last(df_working_s[df_working_s.index <= target_dt - timedelta(hours=h)]["value"])

            feat_s_dict = {
                "heure_sin": hs, "heure_cos": hc, "mois_sin": ms, "mois_cos": mc, "jour_sin": js, "jour_cos": jc,
                "lag_48": sol_lag(24),
            ...
            # Ordre des colonnes exact attendu par XGBoost V2
            feature_cols_sol = ['heure_sin', 'heure_cos', 'mois_sin', 'mois_cos', 'jour_sin', 'jour_cos', 'lag_48', 'rolling_mean_48']
            for region in ["NouvAquitaine", "Occitanie", "PACA", "AuvergneRA"]:
                feature_cols_sol.extend([f'rad_short_{region}', f'rad_dir_{region}', f'cloud_{region}', f'temp_{region}'])
                
            feat_s = pd.DataFrame([feat_s_dict])[feature_cols_sol].fillna(0.0)

            # Prédiction du Load Factor (clipsé entre 0% et 100%)
            pred_s_lf = max(0.0, min(1.0, float(model_solaire.predict(feat_s.astype(float))[0])))
            # Reconversion en Mégawatts pour le réseau réel
        ...
        for chunk in range(total_chunks):
            ...
            chunk_idx = pd.date_range(start=start_hist_dt, end=end_pred_dt, freq="30min", inclusive="right")
            ...
            weather_cols = [c for c in df_tft.columns if any(x in c for x in ["wind_speed", "temp", "pres", "wind_dir", "pc_", "power_density"])]
            ...
            for step in range(steps_in_chunk):
                ...
                val_mw = max(0, pred_lf[step] * CURRENT_CAPACITY)
        ...
        # On colle l'historique figé avec la nouvelle prévision
        if old_run:
        ...
        with tab1:
            st.plotly_chart(make_fig(
                ...
                "#f0f8e8",   # crème blanc — historique conso
                "#d8c830",   # jaune printemps — prévision
                "Consommation nationale",
                last_known_dt, cmp_conso
            ), use_container_width=True)

        with tab2:
            st.plotly_chart(make_fig(
                recent_eol, df_eol["Prédiction (MW)"],
                "#a8e060",   # vert vif printemps — historique éolien
                "#d8c830",   # jaune printemps — prévision
                "Production éolienne — PyTorch TFT · 1M params",
                last_known_dt, cmp_eol
            ), use_container_width=True)

        with tab3:
            st.plotly_chart(make_fig(
                recent_sol, df_sol["Prédiction (MW)"],
                "#d8c830",   # jaune printemps — historique solaire
                "#a8e060",   # vert vif — prévision
                "Production solaire",
                last_known_dt, cmp_sol
            ), use_container_width=True)

        with tab4:
            st.plotly_chart(make_fig(
                recent_residuelle, df_residuelle["Prédiction (MW)"],
                "#c0dfa8",   # vert clair — historique résiduelle
                "#d8c830",   # jaune printemps — prévision
                "Charge résiduelle",
                last_known_dt, cmp_res
            ), use_container_width=True)
